...
def process_file(
    ...
    def normalize_lang(lang_input) -> str:
        ...
        # If it's a full name (e.g., "french"), map it to a code ("fr")
        if s in LANG_NAME_TO_CODE:
            return LANG_NAME_TO_CODE[s]

        # Otherwise, assume it's already a code and clean it (e.g., "fr-FR" -> "fr")
        for sep in ("-", "_"):
            if sep in s:
                s = s.split(sep, 1)[0]
        return s
    ...
    # iterate for each variant
    for variant in range(1, num_variants + 1):
        target_col = (
            report_col if (in_place and variant == 1)
            else f"{report_col}{new_column_prefix}{variant}"
        )
...
def process_file(
    ...
    # Get English index once if needed
    ...
    for variant in range(1, num_variants + 1):
        ...
        # A single loop over rows
        for i, row in df.iterrows():
            # Process main report column
            text = row[report_col]
            lang = row[lang_col]
        ...
        # Assign new columns after the loop
    ...
    if output_path:
        if output_path.endswith(".jsonl"):
            ...
            # Default to JSONL if output format is ambiguous
...
if __name__ == "__main__":
    # The main function now processes .xlsx or .jsonl files.
    # Make sure you have a 'reports.jsonl' file for this example to run.
    # Example line for reports.jsonl:
    # {"report": "Intégrité de la charnière cervico-thoracique.", "lang": "french", "translation": "Integrity of the cervicothoracic junction."}

    df = process_file(
        input_path="parrot_proofread.jsonl",
        output_path="reports_dmeta.jsonl",
        report_col="report_proofread",
        lang_col="language",
        translation_col="translation_proofread",
        num_variants=1,
        in_place=False,
        seed=None,
        max_words_per_lang=20_000,
        use_fuzzy=False,
    )
